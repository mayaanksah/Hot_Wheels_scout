...
def format_alert(product: dict, kind: str, added_to_cart: bool) -> str:
...
async def send_product_alert(
    telegram: tuple[str, str], product: dict, kind: str, added_to_cart: bool
) -> None:
    bot_token, chat_id = telegram
    await send_telegram(
        bot_token, chat_id,
        format_alert(product, kind, added_to_cart),
